<?php
    ...
    $sql="select bt.comuna,count(bt.comuna) cantidad from tbl_pendiente_blindaje_temp bt where ubicacion='Nivel 4' group by bt.comuna";
    ...
    while ($row = mysqli_fetch_object($query)){
        ...
        if ($q->q > 0){
            ...
            $sql="select bloque,idusuario from tbl_blindaje_bloque_ejecutivo where bloque='$row->comuna' And tarea='Nivel 4' And estado=1";
            ...
            while ($r_comuna = mysqli_fetch_object($query_3)){
                ...
                $sql="Select RUT,DESC_ACTIV,LOCALIDAD,NMRO_ORDEN,FECHA_COMPROMISO,CODI_HORARIO,CONTEXTO,ESTD_ACTIV,DESC_AREAFUN,FECHA_OT,COMUNA,'$r_comuna->idusuario',ACTIVIDAD,NODO,SUBNODO,MARCA,ubicacion from tbl_pendiente_blindaje_temp where ubicacion='Nivel 4' And comuna='".$r_comuna->bloque."' limit $i,$resultd";    
    ...
    $sql="Select count(*) q from tbl_pendiente_blindaje_temp where ubicacion='Nivel 4' limit 1";
    $query=mysqli_query($link,$sql);
    ...
    if ($q->q != 0)
    {
        include('listar_dist_ordenes_cargadas_n4.php');
    }
