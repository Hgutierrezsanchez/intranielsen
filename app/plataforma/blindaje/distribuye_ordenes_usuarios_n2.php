<?php
    ...
    $sql="select bt.comuna,count(bt.comuna) cantidad from tbl_pendiente_blindaje bt where FINAL<>'FINALIZADA' and ubicacion='Nivel 2' and ejecutivo='' group by bt.comuna";
    ...
    while ($row = mysqli_fetch_object($query)){
        ...
        if ($q->q > 0){
            ...
            $sql="select idusuario from tbl_blindaje_bloque_ejecutivo where bloque='$row->comuna' And tarea='Nivel 2' And estado=1";
            $query_3=mysqli_query($link,$sql);
            $i=($q->q)-1;
            while ($r_comuna = mysqli_fetch_object($query_3)){
                $sql="Select id From tbl_pendiente_blindaje Where FINAL<>'FINALIZADA' and comuna='$row->comuna' and ejecutivo='' and ubicacion='Nivel 2' limit $i,$resultd";
    ...
    $sql="Select count(*) q from tbl_pendiente_blindaje where ubicacion='Nivel 2' and ejecutivo=''";
    $query=mysqli_query($link,$sql);
    ...
    if ($q->q != 0)
    {
        include('listar_dist_ordenes_sinasignar_n2.php');
    }
?>