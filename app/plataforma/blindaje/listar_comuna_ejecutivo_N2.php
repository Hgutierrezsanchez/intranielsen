...
if(isset($_POST['comuna'])) { 
    ...
    echo '<table class="table table-bordered table-striped">';
        ...
        echo '</thead>';
        echo '<tbody>';
            $sql="SELECT nombre,idusuario,horaingreso,horasalida FROM tblusuario u inner join tblturnos t on t.rut=u.rut Where admin<>1 And t.fecha=date(now()) and upper(tarea)='BLINDAJE N2' And u.estado=1 order by horaingreso,nombre"; 
            ...
            while($row = mysqli_fetch_object($query)){
                echo "<tr>";
                    echo "<td>";
                        ...
                        if (mysqli_num_rows($query_2))
                        {
                            ...
                            if ($r_estado->estado == 1)
                                echo "<input type='checkbox' id='check-n2".$row->idusuario."' onclick=\"des_habilita_usuario_bloque('".$row->idusuario."','".$comuna."','".$NIVEL."')\" checked> &nbsp;";
                            else
                                echo "<input type='checkbox' id='check-n2".$row->idusuario."' onclick=\"des_habilita_usuario_bloque('".$row->idusuario."','".$comuna."','".$NIVEL."')\"> &nbsp;";  
                        }
                        else{
                            echo "<input type='checkbox' id='check-n2".$row->idusuario."' onclick=\"des_habilita_usuario_bloque('".$row->idusuario."','".$comuna."','".$NIVEL."')\"> &nbsp;";  
                        }
                    ...
                    echo "</td>";
                    echo "<td><div id='div-n2".$row->idusuario."' >";
