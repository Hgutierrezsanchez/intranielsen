...
$consulta1 = "SELECT u.nombre, h.idhe, h.rut, h.fecha, h.desde, h.hasta, h.motivo, us.nombre as solicitante, h.estado, cast(create_at as DATE) creada FROM (tblhorasextras h INNER JOIN tblusuario u ON u.rut = h.rut) inner join tblusuario us on h.solicitante=us.idusuario where h.estado = '$estado' And CAST( h.create_at AS DATE ) BETWEEN '$fdesde' AND '$fhasta' and h.solicitante in (select idusuarioejecutivo from tblsuperejecutivo where idusuariosuper='".$iduser."')";
            ...
            <tbody>
                <form name="updatehe_form" action="" method="POST">
                    ...
                    while($row = mysqli_fetch_array($sql))
                    {
                        ...
                        echo "      <td>";
                        echo "          <p><input type=\"submit\" name=\"Submit\" value=\" Revisar \" onclick=\"updatehe('".$row['idhe']."')\">";
                        echo "      </td>";
                        echo "	</tr>";
                    }
                    mysqli_close($link);
